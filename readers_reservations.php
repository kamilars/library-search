...
$query_show_all_reserved_books= "SELECT * FROM lib_orders WHERE reader_login='".$_SESSION['login']."' ORDER BY reservation_date, reservation_time ";
	...
	{
	echo "<br><div id='counter'> <font color='white' align='center'> Всего заказано книг: $rows</font></div><br>";
	
	while ($row=mysqli_fetch_row($result_query_show_all_reserved_books))
	{	$order_number=$row[0];
		echo '<table><tr>';
		if ($row[9]=="not_seen") echo '<td rowspan="4" id="status" class="not_seen"><b>Статус: </b>не просмотрено библиотекарем</td>';
		elseif ($row[9]=="not_prepared") echo '<td rowspan="4" id="status" class="not_prepared"><b>Статус: </b>на стадии подготовка</td>';
		elseif ($row[9]=="not_found") echo '<td rowspan="4" id="status" class="not_found"><b>Статус: </b>книга не найдена</td>';		
		elseif ($row[9]=="prepared") echo '<td rowspan="4" id="status" class="prepared"><b>Статус: </b>готово к выдаче</td>';
	echo '<td rowspan="2" id="datetime"><b>Дата:</b> ';
	$date = DateTime::createFromFormat('Y-m-d', $row[6]);
	echo $date->format('d F Y');;
	echo'<br></td><td id="book_name">';
	echo $row[3];
	...
	echo '	</td><td id="action"><form method="post" name="delete_reservation" action="readers_reservations.php">
			...
			<input type="submit" name="'.$order_number.'" id="btn_del" value="Отменить заказ">
			</form></td></tr>
